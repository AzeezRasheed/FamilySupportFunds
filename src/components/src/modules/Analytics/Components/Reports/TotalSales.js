...
const TotalSales = ({ location, distributor, getSingleDistributor }) => {
  ...
  // allSystemOrders && console.log(allSystemOrders[1][0]?.price);
  ...
  let dist_completed_orders = filter(uniqueAllOrders, function (order) {
    return (
    ...
    <Dashboard location={location}>
      <div className="px-10 pt-10 pb-15 md:pb-10 lg:pb-10">
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-16 items-center">
            {allSystemOrders && <CSVLink
              data={getData(allSystemOrders[0])}
              filename={`${
                getDistributorName(Dist_Code)?.SYS_Code
                }_sales_report.csv`}
              style={{ textDecoration: "none" }}
