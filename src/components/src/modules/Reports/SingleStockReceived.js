...
import { filter, uniqBy, flatten, orderBy } from "lodash";
...
const SingleStockReceived = ({ location }) => {
  ...
  inventoryData.map((sku) => {
    data.push({
      ...
      Remarks: remarks,
      Date: date,
      Total_Empties_Returned: empties,
    });
  });
  const backButton = () => {
    history.goBack();
    ...
    <Dashboard location={location}>
      ...
      >
        <div>
          ...
          <p style={{ marginBottom: 10 }}>
            <span style={{ fontWeight: "bold" }}>{t("empties_returned")}: </span>
            {empties}
          </p>
