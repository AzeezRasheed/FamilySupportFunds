...
import { customerNet, userNet } from "../../../utils/urls";
import { getAllDistributors, setChangeDistOverlay } from "../KPO/actions/UsersAction";
import { getPaginatedCustomers } from "./actions/customer";


const ChangeDistributor = ({ action, currentPage, currentTable, id, customerData }) => {
  ...
  const customer_id = useSelector((state) => state.AllUsersReducer.customer_id);

  const thisCust = currentTable?.filter((user) => user.id === customer_id);

  const dist_CODES = thisCust?.length
    ? thisCust[0]?.DIST_Code
    : [0];
  ...
  const saveDist = () => {
    ...
    selectedDist &&
      selectedDist?.forEach((dist) => selectedDistID.push(dist?.SYS_Code));
    
    let params = {
      sfCode: thisCust[0]?.SF_Code,
      distCode: selectedDistID[0],
    };
    customerApi
      .patch("updatecustomer/update-dist/change-dist", params)
      .then((result) => {
        //update vehicle
        dispatch(getPaginatedCustomers(country, currentPage, customerData))
        dispatch(getAllDistributors(country));
      });
    // dispatch(saveDist(id, params, selectedDist))
    dispatch(setChangeDistOverlay(false));
  };
                ...
                >
                  ...
                  <div className="mt-2 px-12">
                    <div className="justify-between items-center mb-2">
                      ...
                      >
                        Reassign {thisCust[0]?.CUST_Name} to another Distributor
                      </p>
                      <div
                        style={{
                          marginBottom: 24,
