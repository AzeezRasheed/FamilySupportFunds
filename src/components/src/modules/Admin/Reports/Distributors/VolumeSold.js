...
const VolumeSold = ({ location, distributor, getSingleDistributor }) => {
  ...
  completedOrders.map((item, index) => {
    thisItem = filter(product, { productId: item.productId });
    if (!thisItem.length) {
      let x = filter(completedOrders, { productId: item.productId });
      totalP = parseFloat(totalP) + parseFloat(item.price);
      product.push({
      ...
      <div className="px-10 pt-10 pb-15 md:pb-10 lg:pb-10">
        ...
        <div className="flex justify-between">
          <div className="flex gap-2 items-center font-customGilroy text-sm not-italic mt-4 mb-10">
            ...
            /<p className="font-medium text-grey-100">{t("total_volume_sold")}</p>
          </div>
        </div>
        <div>
                    ...
                    <tr key={index}>
                      ...
                      <td className="font-customGilroy text-sm font-medium text-center align-middle p-6">
                        <p
                          // onClick={() => {
                          //   history.push(
                          //     `/dashboard/productvolumesold/${code}/${
                          //       getProductDetails(product.productId)?.productId
