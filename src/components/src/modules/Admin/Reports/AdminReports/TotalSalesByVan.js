...
const TotalSalesByVan = ({ location, distributor }) => {
  ...
  useEffect(() => {
    ...
    dispatch(getCountryOrdersByDateRange(startRange, stopRange, countryCode(country), "Van-Sales"));
  }, [startRange, stopRange]);

  let completedOrders = allSystemOrders?.orders?.length > 0 && filter(allSystemOrders.orders, function (order) {
      return (
...
export default TotalSalesByVan;
