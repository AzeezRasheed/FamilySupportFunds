...
import { getDailyStockReport, setLoadingToDefault, showCalendar } from "../actions/ReportAction";
...
const DailyStockCount = ({ location }) => {
  ...
  useEffect(() => {
    ...
    dispatch(getDailyStockReport(startRange, stopRange, country === "South Africa" ? "SA" : country));
  }, [startRange, stopRange]);

  const daily_stock = useSelector((state) => state.ReportReducer.daily_stock_report);
                  ...
                  {currentTableData().map((stock, index) => (
                    <tr key={index}>
                      ...
                      <td
                        ...
                        className="text-left align-middle cursor-pointer pl-4 pr-10"
                      >
                        <p className="font-customGilroy text-sm text-red-900 font-medium">
                          {stock.distributor}
                        </p>
                      </td>
                      <td className="font-customGilroy text-sm font-medium align-middle py-6 px-10 w-56">
